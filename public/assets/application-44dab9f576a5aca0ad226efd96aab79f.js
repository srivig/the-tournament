...
(function() {
  var disable_winner_select, enable_winner_select, judge_winner, reset_winner, set_winner;

  judge_winner = function(score_1, score_2) {
    var winner;
    winner = null;
    if (score_1 > score_2) {
      winner = 1;
  ...
  reset_winner = function() {
    $('.game_game_records_winner input').attr('value', '');
    $('.panel').removeClass('panel-warning').addClass('panel-default');
    return $('.panel-heading i').removeClass('fa-trophy fa-times');
  };

  set_winner = function(winner) {
    ...
    return $('.panel-heading i').eq(2 - winner).addClass('fa-times');
  };

  enable_winner_select = function() {
    return $('#winner-select').removeAttr('disabled');
  };

  disable_winner_select = function() {
    $('#winner-select').attr('disabled', 'disabled');
    return $('#winner-select').val('0');
  };
  ...
  $(function() {
    ...
    if ($('#game-edit').length) {
      ...
      $('.game_game_records_score input').change(function() {
        var winner;
        scores = [Number($('.game_game_records_score input')[0].value), Number($('.game_game_records_score input')[1].value)];
        ...
        if (winner) {
          set_winner(winner);
          return disable_winner_select();
        } else {
          return enable_winner_select();
        }
      });
      return $('#winner-select').change(function() {
        var winner;
        reset_winner();
        if ($.inArray($(this).val(), ["1", "2"]) >= 0) {
          winner = $(this).val();
          return set_winner(winner);
        }
      });
    }
  });
...
(function() {
  $(function() {
    var addCountryFlg, createBracket, hideDecimal, prepareImage;
        ...
        return setTimeout(function() {
          return html2canvas($(".bracket"), {
            ...
            onrendered: function(canvas) {
              ...
              return $("#download_btn, #btn-upload_img").button("reset");
            }
          });
        }, 1500);
      };
    ...
    if ($('body').data('controller') === 'tournaments' && $('body').data('action') === 'photos') {
      if (gon.album_id !== null) {
        return window.fbAsyncInit = function() {
          ...
          return FB.api(request_url, function(response) {
            var album_url, photos;
            photos = '';
            album_url = "https://www.facebook.com/media/set/?set=a." + gon.album_id;
            $.each(response.data, function(index, obj) {
              var end_div, start_div;
              if (index >= 12) {
                return false;
              }
              start_div = end_div = '';
              ...
              return photos += start_div + '<div class="col-sm-3"><a href="' + album_url + '" target="_blank"><div style="width:100%;height:200px;background-size:cover;background-position:center;background-image:url(' + obj.images[0]['source'] + ')"></div></a></div>' + end_div;
            });
            return $('.album-container').append(photos);
          });
        };
      }
    }
  ...
  $(function() {
    if ($('body').data('controller') === 'users' && $('body').data('action') === 'show') {
      return window.fbAsyncInit = function() {
        ...
        return FB.api(request_url, function(response) {
          return $('.cover').css('background-image', "url('" + response.cover.source + "')").show();
        });
      };
    }
  });
